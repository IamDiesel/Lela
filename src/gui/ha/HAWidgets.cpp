...
HASensorWidget::HASensorWidget(lv_obj_t* parent, int tab_idx, String type, String entity, int x, int y, int w, int h, const char* name, const char* mdi, const char* c_on, const char* c_off, bool showChart, int chart_w, int chart_h, int chart_x, int chart_y, String c_min, String c_max)
    : HAWidget(parent, tab_idx, type, entity, x, y, w, h, name, mdi, c_on, c_off) {
    this->show_chart = showChart; this->chart_w_pct = chart_w; this->chart_h_pct = chart_h;
    this->chart_x_ofs = chart_x; this->chart_y_ofs = chart_y; this->chart_min = c_min; this->chart_max = c_max;
    this->chart = nullptr; this->state_label = nullptr; this->unit_label = nullptr; this->ser = nullptr;
    this->current_min = 0; this->current_max = 100; this->first_value_received = false;
    this->last_value = 0; this->last_update_millis = millis();
    
    time_t now; time(&now);
    for(int i=0; i<50; i++) { timestamps[i] = now - ((49 - i) * 10); is_held[i] = true; }
    hold_timer = lv_timer_create(hold_timer_cb, 10000, this);
    buildUI(); updateState("unknown");
}

HASensorWidget::~HASensorWidget() {
    if (hold_timer) { lv_timer_del(hold_timer); hold_timer = nullptr; }
}
...
void HASensorWidget::buildUI() {
    if (show_chart) {
        ...
        lv_obj_set_style_pad_left(chart, 35, 0); lv_obj_set_style_pad_bottom(chart, 25, 0); 
        lv_obj_set_style_pad_right(chart, 10, 0); lv_obj_set_style_pad_top(chart, 10, 0);
        lv_obj_align(chart, LV_ALIGN_CENTER, chart_x_ofs, chart_y_ofs);
        lv_chart_set_type(chart, LV_CHART_TYPE_LINE); lv_chart_set_update_mode(chart, LV_CHART_UPDATE_MODE_SHIFT);
        ...
        lv_obj_set_style_bg_opa(chart, 0, LV_PART_MAIN); lv_obj_set_style_border_width(chart, 0, LV_PART_MAIN);
        lv_obj_set_style_line_color(chart, lv_color_hex(0x333333), LV_PART_MAIN); 
        lv_obj_set_style_line_width(chart, 2, LV_PART_TICKS); lv_obj_set_style_line_color(chart, lv_color_hex(0x666666), LV_PART_TICKS);
        lv_obj_set_style_text_color(chart, lv_color_hex(0xAAAAAA), LV_PART_TICKS); lv_obj_set_style_text_font(chart, &lv_font_montserrat_12, LV_PART_TICKS);
        lv_obj_set_style_line_width(chart, 3, LV_PART_ITEMS); lv_obj_set_style_bg_opa(chart, LV_OPA_30, LV_PART_ITEMS); lv_obj_set_style_size(chart, 6, LV_PART_INDICATOR); 
        ...
        lv_obj_set_style_text_font(state_label, &lv_font_montserrat_16, 0); lv_obj_set_style_text_color(state_label, lv_color_white(), 0);
        lv_label_set_text(state_label, "--"); lv_obj_align(state_label, LV_ALIGN_TOP_RIGHT, -10, 20);
        
        unit_label = lv_label_create(container);
        lv_obj_set_style_text_font(unit_label, &lv_font_montserrat_14, 0); lv_obj_set_style_text_color(unit_label, lv_color_hex(0x888888), 0);
        lv_label_set_text(unit_label, ""); lv_obj_align(unit_label, LV_ALIGN_TOP_LEFT, 15, 20);
    } else {
        lv_obj_clear_flag(icon_label, LV_OBJ_FLAG_HIDDEN); 
        state_label = lv_label_create(container);
        lv_obj_set_style_text_font(state_label, &lv_font_montserrat_28, 0); lv_obj_set_style_text_color(state_label, lv_color_white(), 0);
...
void HASensorWidget::setChartConfig(bool show, int w_p, int h_p, int x_ofs, int y_ofs, String c_min, String c_max) {
    if (this->show_chart == show && this->chart_w_pct == w_p && this->chart_h_pct == h_p && this->chart_x_ofs == x_ofs && this->chart_y_ofs == y_ofs && this->chart_min == c_min && this->chart_max == c_max) return;
    this->show_chart = show; this->chart_w_pct = w_p; this->chart_h_pct = h_p; this->chart_x_ofs = x_ofs; this->chart_y_ofs = y_ofs; this->chart_min = c_min; this->chart_max = c_max;
    ...
    first_value_received = false; time_t now; time(&now);
    for(int i=0; i<50; i++) { timestamps[i] = now - ((49 - i) * 10); is_held[i] = true; }
    buildUI(); String old = current_state; current_state = "force_update"; updateState(old);
}

void HASensorWidget::hold_timer_cb(lv_timer_t * t) {
    HASensorWidget* w = (HASensorWidget*)t->user_data;
    if (!w->first_value_received || !w->show_chart) return;
    if (millis() - w->last_update_millis >= 10000) { w->addChartValue(w->last_value, true); w->last_update_millis = millis(); }
}

void HASensorWidget::addChartValue(float val, bool held) {
    if (!show_chart || !chart || !ser) return;
    for(int i = 0; i < 49; i++) { timestamps[i] = timestamps[i+1]; is_held[i] = is_held[i+1]; }
    time_t now; time(&now); timestamps[49] = now; is_held[49] = held;

    if (!first_value_received) { current_min = val; current_max = val; first_value_received = true; } 
    else { if (val < current_min) current_min = val; if (val > current_max) current_max = val; }
    ...
    if (chart_min.length() > 0 && chart_max.length() > 0) {
        ...
        if (range < 0.5f) lv_chart_set_range(chart, LV_CHART_AXIS_PRIMARY_Y, (lv_coord_t)((current_min - 0.5f) * 10.0f), (lv_coord_t)((current_max + 0.5f) * 10.0f));
        else { float pad = range * 0.2f; lv_chart_set_range(chart, LV_CHART_AXIS_PRIMARY_Y, (lv_coord_t)((current_min - pad) * 10.0f), (lv_coord_t)((current_max + pad) * 10.0f)); }
    }
    ...
    if (dsc->part == LV_PART_TICKS && dsc->id == LV_CHART_AXIS_PRIMARY_X) {
        if (dsc->text) {
            ...
            if (pt_idx >= 0 && pt_idx < 50) {
                ...
                if (t > 1000000) { struct tm * tm_info = localtime(&t); snprintf(dsc->text, dsc->text_length, "%02d:%02d", tm_info->tm_hour, tm_info->tm_min); } 
                else snprintf(dsc->text, dsc->text_length, "--:--"); 
            }
        }
    }
    else if (dsc->part == LV_PART_TICKS && dsc->id == LV_CHART_AXIS_PRIMARY_Y) {
        if (dsc->text) snprintf(dsc->text, dsc->text_length, "%.1f", (float)dsc->value / 10.0f);
    }
    else if (dsc->part == LV_PART_INDICATOR) {
        int pt_idx = dsc->id; 
        if (pt_idx >= 0 && pt_idx < 50) {
            if (w->is_held[pt_idx]) { dsc->rect_dsc->bg_opa = LV_OPA_TRANSP; dsc->rect_dsc->border_opa = LV_OPA_TRANSP; } 
            else { dsc->rect_dsc->bg_opa = LV_OPA_COVER; dsc->rect_dsc->border_opa = LV_OPA_COVER; }
        }
    }
}
...
void HASensorWidget::setAlignments(int i_align, int t_align, int s_align, int i_margin, int t_margin, int s_margin) {
    ...
    auto apply = [](lv_obj_t* obj, int align_val, int margin) {
        if (!obj) return; int ox = 0, oy = 0;
        switch(align_val) { case LV_ALIGN_TOP_MID: oy = margin; break; case LV_ALIGN_BOTTOM_MID: oy = -margin; break; case LV_ALIGN_LEFT_MID: ox = margin; break; case LV_ALIGN_RIGHT_MID: ox = -margin; break; case LV_ALIGN_CENTER: oy = margin; break; }
        lv_obj_align(obj, align_val, ox, oy);
    };
...
void HASensorWidget::updateState(String state) {
    ...
    HAWidget::updateState(state);
    uint32_t c_on_val = 0x3498DB; if (color_on.length() > 0 && color_on.startsWith("#")) c_on_val = strtol(color_on.substring(1).c_str(), NULL, 16);
    
    if (!show_chart) lv_obj_set_style_text_color(icon_label, lv_color_hex(c_on_val), 0);
    else if (ser) { lv_chart_set_series_color(chart, ser, lv_color_hex(c_on_val)); lv_obj_set_style_bg_color(chart, lv_color_hex(c_on_val), LV_PART_ITEMS); }
    
    String unit = HaWebsocketLogic_GetUnit(entity_id); String display_text = state;
    if (unit.length() > 0 && state != "unavailable" && state != "unknown") {
        display_text += " " + unit;
        if (!show_chart) { lv_obj_set_style_text_font(icon_label, &lv_font_montserrat_48, 0); lv_label_set_text(icon_label, unit.c_str()); } 
        else if (unit_label) lv_label_set_text_fmt(unit_label, "[%s]", unit.c_str());
    } else {
        if (!show_chart) { lv_obj_set_style_text_font(icon_label, &lela_icons, 0); lv_label_set_text(icon_label, getIconForEntity(entity_id, mdi_icon).c_str()); }
    }
    
    lv_label_set_text(state_label, display_text.c_str());
    if (show_chart && state != "unavailable" && state != "unknown") {
        float val = state.toFloat(); last_value = val; last_update_millis = millis(); addChartValue(val, false); 
    }
}
...
void HAActionWidget::updateState(String state) {
    if (this->current_state == state && lv_obj_get_style_text_opa(icon_label, 0) == 255) return; 
    this->current_state = state; lv_obj_set_style_text_opa(icon_label, 255, 0); 
...
void HAMediaWidget::updateState(String state) {
    if (this->current_state == state && lv_obj_get_style_text_opa(icon_label, 0) == 255) return; 
    HAWidget::updateState(state); lv_obj_set_style_text_opa(icon_label, 255, 0); 
...
void HAMediaWidget::onClick() { lv_obj_set_style_text_opa(icon_label, 127, 0); HaWebsocketLogic_CallService("media_player", "toggle", entity_id); }
...
void HAMediaItemWidget::onClick() { lv_obj_set_style_text_opa(icon_label, 127, 0); HaWebsocketLogic_CallPlayMedia(entity_id, media_content_type, media_content_id); }

// =========================================================
// VACUUM WIDGET (KOMPAKT & EINSTELLBAR)
// =========================================================
HAVacuumWidget::HAVacuumWidget(lv_obj_t* parent, int tab_idx, String type, String entity, int x, int y, int w, int h, const char* name, const char* mdi, const char* c_on, const char* c_off)
    ...
    // FIX: Korrekter Aufruf der Base-Class Methode!
    setAlignments(icon_align, text_align, state_align, icon_margin, text_margin, state_margin);

    lv_obj_set_style_text_opa(icon_label, 80, 0); // Transparent, damit Buttons sichtbar bleiben
    ...
    // Standardmaessige Button-Groeßen, wenn noch nichts konfiguriert wurde
    if (chart_w_pct <= 0 || chart_w_pct == 95) { 
        chart_w_pct = 60; // Button Breite
        chart_h_pct = 40; // Button Hoehe
        chart_x_ofs = 10; // Abstand dazwischen
        chart_y_ofs = 10; // Y-Offset von unten
    }
...
void HAVacuumWidget::updateState(String state) {
    if (this->current_state == state && lv_obj_get_style_text_color(icon_label, 0).full != 0) return; 
    ...
    uint32_t c_on_val = 0x1ABC9C; if (color_on.length() > 0 && color_on.startsWith("#")) c_on_val = strtol(color_on.substring(1).c_str(), NULL, 16);
    uint32_t c_off_val = 0x555555; if (color_off.length() > 0 && color_off.startsWith("#")) c_off_val = strtol(color_off.substring(1).c_str(), NULL, 16);
