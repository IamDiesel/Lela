...
String getIconForEntity(String entity_id, String mdi_icon) {
    ...
    if (entity_id.startsWith("switch.") || entity_id.startsWith("input_boolean.") || entity_id.startsWith("button.") || entity_id.startsWith("input_button.")) {
        return getSafeIcon("mdi:power"); 
    }
    if (entity_id.startsWith("script.") || entity_id.startsWith("automation.") || entity_id.startsWith("scene.")) {
        return getSafeIcon("mdi:play");
    }
    if (entity_id.startsWith("light.")) {
        return getSafeIcon("mdi:lightbulb"); 
    }
    if (entity_id.startsWith("media_player.")) {
        return getSafeIcon("mdi:television");
    }
    if (entity_id.startsWith("camera.")) {
        return getSafeIcon("mdi:cctv");
    }
    if (entity_id.startsWith("climate.") || entity_id.indexOf("temp") != -1) {
        return getSafeIcon("mdi:thermometer");
    }
    if (entity_id.startsWith("vacuum.")) {
        return getSafeIcon("mdi:robot-vacuum");
    }
    if (entity_id.indexOf("battery") != -1) {
        return getSafeIcon("mdi:battery");
    }
    if (entity_id.indexOf("solar") != -1) {
        return getSafeIcon("mdi:solar-panel");
    }
    if (entity_id.indexOf("katze") != -1 || entity_id.indexOf("cat") != -1) {
        return getSafeIcon("mdi:cat");
    }
    if (entity_id.indexOf("kaffee") != -1 || entity_id.indexOf("coffee") != -1) {
        return getSafeIcon("mdi:coffee");
    }
...
HAWidget::HAWidget(lv_obj_t* parent, int tab_idx, String type, String entity, int x, int y, int w, int h, const char* name, const char* mdi, const char* c_on, const char* c_off) {
    this->tab_index = tab_idx; 
    this->type = type; 
    this->entity_id = entity; 
    this->mdi_icon = mdi;
    this->color_on = c_on; 
    this->color_off = c_off; 
    this->widget_name = formatEntityName(entity_id, String(name));
    this->current_state = "unknown";
    
    this->icon_align = LV_ALIGN_TOP_MID; 
    this->text_align = LV_ALIGN_BOTTOM_MID; 
    this->state_align = LV_ALIGN_CENTER;
    this->icon_margin = 5; 
    this->text_margin = 5; 
    this->state_margin = 0;
...
void HAWidget::setAlignments(int i_align, int t_align, int s_align, int i_margin, int t_margin, int s_margin) {
    ...
    text_align = t_align; 
    state_align = s_align;
    icon_margin = i_margin; 
    text_margin = t_margin; 
    state_margin = s_margin;
...
void HAWidget::setName(String n) { 
    widget_name = formatEntityName(entity_id, n); 
    lv_label_set_text(name_label, widget_name.c_str()); 
}

void HAWidget::setMdiIcon(String mdi) { 
    mdi_icon = mdi; 
    lv_label_set_text(icon_label, getIconForEntity(entity_id, mdi_icon).c_str()); 
}

void HAWidget::setColors(String on, String off) { 
    color_on = on; 
    color_off = off; 
...
void HAWidget::setPosition(int x, int y, bool snap) { 
...
void HAWidget::setSnapToGrid(bool snap) { 
    this->snap_to_grid = snap; 
}

void HAWidget::setChartConfig(bool show, int w_p, int h_p, int x_ofs, int y_ofs, String c_min, String c_max) {
    show_chart = show; 
    chart_w_pct = w_p; 
    chart_h_pct = h_p; 
    chart_x_ofs = x_ofs; 
...
String HAWidget::getEntityId() { return entity_id; }
String HAWidget::getType() { return type; }
String HAWidget::getName() { return widget_name; }
String HAWidget::getMdiIcon() { return mdi_icon; }
String HAWidget::getColorOn() { return color_on; }
String HAWidget::getColorOff() { return color_off; }
int HAWidget::getX() { return lv_obj_get_x(container); }
int HAWidget::getY() { return lv_obj_get_y(container); }
int HAWidget::getW() { return lv_obj_get_width(container); }
int HAWidget::getH() { return lv_obj_get_height(container); }
int HAWidget::getTabIndex() { return tab_index; }

int HAWidget::getIconAlign() { return icon_align; }
int HAWidget::getTextAlign() { return text_align; }
int HAWidget::getStateAlign() { return state_align; }
int HAWidget::getIconMargin() { return icon_margin; }
int HAWidget::getTextMargin() { return text_margin; }
int HAWidget::getStateMargin() { return state_margin; }
bool HAWidget::getSnapToGrid() { return snap_to_grid; }

bool HAWidget::getShowChart() { return show_chart; }
int HAWidget::getChartWPct() { return chart_w_pct; }
int HAWidget::getChartHPct() { return chart_h_pct; }
int HAWidget::getChartXOfs() { return chart_x_ofs; }
int HAWidget::getChartYOfs() { return chart_y_ofs; }
String HAWidget::getChartMin() { return chart_min; }
String HAWidget::getChartMax() { return chart_max; }
String HAWidget::getMediaContentType() { return ""; }
String HAWidget::getMediaContentId() { return ""; }
